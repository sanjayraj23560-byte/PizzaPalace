...
export default function ChatBox() {
    ...
    return (
        // Outer Wrapper centered over full screen with dark background
        <div className="flex justify-center items-center w-full min-h-screen p-4 bg-slate-950">
            ...
            <div className="relative w-full max-w-md bg-violet-950/30 border border-violet-500/30 rounded-3xl shadow-[0_8px_32px_0_rgba(124,58,237,0.2)] backdrop-blur-2xl flex flex-col h-137.5 overflow-hidden">
                ...
                {/* Chat Header */}
                <div className="px-5 py-4 bg-violet-950/40 border-b border-violet-500/20 backdrop-blur-md flex items-center justify-between z-10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-violet-300/70">Always active to assist you</p>
                        </div>
                    </div>
                </div>
                    ...
                    {messages.length === 0 ? (
                        ...
                        messages.map((m, i) => (
                            ...
                            >
                                ...
                                {/* Message Bubble */}
                                <div
                                    className={`p-3.5 rounded-2xl max-w-[80%] text-sm leading-relaxed transition-all ${m.role === 'user'
                                            ? 'bg-linear-to-r from-violet-600 to-purple-600 text-white rounded-tr-none shadow-md shadow-violet-900/30 border border-violet-400/30'
                                            : 'bg-violet-900/40 text-violet-100 rounded-tl-none border border-violet-500/30 backdrop-blur-md shadow-inner'
                                        }`}
                                >
                                    {m.content}
                                </div>
                ...
                {/* Input Bar */}
                <div className="p-3 bg-violet-950/50 border-t border-violet-500/20 backdrop-blur-md z-10">
                    <div className="flex gap-2 items-center bg-violet-950/80 border border-violet-500/30 rounded-xl p-1.5 focus-within:border-violet-400 focus-within:shadow-[0_0_15px_rgba(139,92,246,0.3)] transition-all">
                        <input
                            value={input}
                        ...
                        <button
                            ...
                            className="bg-violet-600 hover:bg-violet-500 disabled:opacity-30 disabled:hover:bg-violet-600 text-white p-2 rounded-lg transition-all shrink-0 shadow-md shadow-violet-900/40 active:scale-95"
                        >
                            <Send size={16} />
                        </button>
                    </div>
